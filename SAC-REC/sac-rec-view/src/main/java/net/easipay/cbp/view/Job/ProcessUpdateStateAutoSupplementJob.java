...
@GlassJob(description = "更新内部对账完成自动增补遗漏差错交易状态任务", team = "对账系统", created = "2016-01-19")
@PersistJobDataAfterExecution
@DisallowConcurrentExecution
public class ProcessUpdateStateAutoSupplementJob implements Job {
  @Autowired
  RecDiffManager recDiffManager;

  private static final Logger logger = Logger.getLogger(ProcessUpdateStateAutoSupplementJob.class);

  @Override
  public void execute(JobExecutionContext context) throws JobExecutionException {
    logger.info("开始执行更新内部对账完成自动增补遗漏差错交易状态任务...");
    try {
      // 获取所有需要补单的差错
      ...
      if (recDiffList != null && !recDiffList.isEmpty()) {
        for (SacRecDifferences sacRecDifferences : recDiffList) {
          try {
            recDiffManager.processUpdateStateAutoSupplementTrxRecDiff(sacRecDifferences);
    ...
    logger.info("结束执行更新内部对账完成自动增补遗漏差错交易状态任务...");
  }
}
