 ...
 * 分页标签
 */
public class PageTagAjax extends TagSupport {
    ...
    private int pageSize = 0; //每一页面显示的最大记录数
    ...
    public int doEndTag() throws JspException {
        ...
        reStr.append("onChange='pageNumber(\"" + urlName + addParams(paramsStr) + "\")'>");
        if (totalPages == 0) {
...
//      return (EVAL_PAGE);
    ...
    private String getUrl(int pageNo, String name) {
        return "【<a href=\"javascript:void(0)\" onclick='send_request(\"" + dealUrl(urlName, pageNo)
                + "\", \"second\")' class=\"splitPage\">" + name + "</a>】";
    }
