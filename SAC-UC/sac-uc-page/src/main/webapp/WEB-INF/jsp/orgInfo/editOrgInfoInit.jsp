...
<!-- Body -->
    ...
    <div class="content">
      ...
      <div class="con ">
				<form:form commandName="ucOrgInfo" action="${ctx}/editOrgInfo" method="POST" enctype="multipart/form-data">   	    
					<div class="table fontcolor4 fontsize1 fontfamily2">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
						   <input name="orgIdString" id ="orgIdString"  value = "${ucOrgInfo.orgId}" type = "hidden"/>
							<tr>
								<td width="12%" align="right">企业组织机构码：</td>
								<td width="40%">
								<span> <form:input path = "orgCode" class="txt2" id="orgCode" value = "${ucOrgInfo.orgCode}" /> </span>
                                <span><form:errors path="orgCode" cssStyle="color:red"/></span>
								</td>
								<td width="12%" align="right">企业名：</td>
								<td width="40%">
								<span> <form:input path="orgName" class="txt2" id="orgName" value = "${ucOrgInfo.orgName}"/> </span>
							...
							<tr>
								...
								<td width="40%">
								<span> <form:input  
										path="shortName" class="txt2" id="shortName" value = "${ucOrgInfo.shortName}"/> </span>
										<span><form:errors path="shortName" cssStyle="color:red"/></span>
								</td>
								<td width="10%" align="right">企业英文名：</td>
								<td width="40%">
								<span> <form:input  
										path="engName" class="txt2" id="engName" value="${ucOrgInfo.engName}"/> </span>
							...
							<tr>
								...
								<select name="orgType" class="select1">
										<c:forEach items="${sysDicOrgTypeList}" var="sys">
											<option value="${sys.dicValue}" <c:if test="${sys.dicValue == ucOrgInfo.orgType}">selected</c:if>  >${sys.dicDesc}</option>
										</c:forEach>
								</select>
								</td>
							</tr>
								...
								<td width="40%">
									<select name="status" id = "status" class="select1">
										<c:forEach items="${sysDicStatusList}" var="sys">
											<option value="${sys.dicValue}" <c:if test="${sys.dicValue==ucOrgInfo.status}">selected</c:if>>${sys.dicDesc}</option>
										</c:forEach>
								</select>
								</td>
							...
							<tr>
								...
								<select name="lawmanPasstype" class="select1">
										<c:forEach items="${sysDicLawanPasstypeList}" var="sys">
											<option value="${sys.dicValue}" <c:if test="${sys.dicValue == ucOrgInfo.lawmanPasstype}">selected</c:if>  >${sys.dicDesc}</option>
										</c:forEach>
								</select>
							...
							<tr>
							<td width="10%" align="right">营业执照扫描件：</td>
					    ...
					    <td style="width: 16%" align="right">机构代码证扫描件：</td>
						<td align="left" style="text-align: left;">
						<!--  <a href="${ctx}/downLoadInit?downloadFile=${ucOrgInfo.orgLicenseFile}">${ucOrgInfo.orgLicenseFile}</a>   -->
					...
					<tr>
					        <td width="10%" align="right">密钥：</td>
								...
								<span> <form:input  
										path="merchantSecure" id="merchantSecure" /> </span>
										<span><form:errors path="merchantSecure" cssStyle="color:red"/></span>
							</td>
					</tr>
							<tr>
								<td align="right">&nbsp;</td>
								<td><input name="submitbtn" class="txt2" type="submit" 
									 value="修改商户" class="btn1"
									id="submitbtn" />
								</td>
							</tr>
