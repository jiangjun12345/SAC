package net.easipay.cbp.dao.base;
...
import net.easipay.cbp.util.OutputLog;
...
public class GenericDaoiBatis<T, PK extends Serializable> implements GenericDao<T, PK> {
    ...
    public T save(final T object) {
        ...
        if (StringUtils.isBlank(keyId)) {
            ...
            {
            	log.error(OutputLog.formatLog("DataAccessException: Date save exception."));
				e.printStackTrace();
			}
    ...
    public T update(final T object) {
        ...
        if (!StringUtils.isBlank(keyId)) {
            ...
            {
				log.error(OutputLog.formatLog("DataAccessException: Date update exception."));
				e.printStackTrace();
			}
        }
	...
	public Page queryByPage(Page page) 
	{
		if(page!=null)
		{
			page.setTotalData(getTotalCount());
			if(page.getTotalData()>0)
			{
				page.setPagingResults(getSqlMapClientTemplate()
				    .queryForList(ClassUtils.getShortName(this.persistentClass), page));
			}
		}
		return page;
	}
	...
	{
		return (Integer) getSqlMapClientTemplate().queryForObject(ClassUtils.getShortName(this.persistentClass));
	}
