...
public class BcBankDataExcel {
	 ...
	 * 2015-07-24
	 */
	private static Logger logger = LoggerFactory.getLogger(BcBankDataExcel.class);
	 ...
	 * @throws IOException
	 */
	public static List<SacReceiveBankRecon> readXlsx(InputStream inputStream,String fileName)
			throws IOException {
		...
		for (int numSheet = 0; numSheet < xssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			for (int rowNum = 1; rowNum <= xssfSheet.getLastRowNum(); rowNum++) {
				...
				if (xssfRow != null) {
					...
					XSSFCell payAmount = xssfRow.getCell(5);
					XSSFCell trxTime = xssfRow.getCell(12);
					XSSFCell txtState = xssfRow.getCell(6);
					...
					//判断正向逆向交易
					if(txtState.toString().equals("已支付")){
	 ...
	 * @throws IOException
	 */
	public static List<SacReceiveBankRecon> readXls(InputStream inputStream,String fileName)
			throws IOException {
		System.out.println("Processing..." + fileName);
		InputStream is = inputStream;
		...
		for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				if (hssfRow != null) {
					...
					HSSFCell bankSerialNo = hssfRow.getCell(0);
					HSSFCell payAmount = hssfRow.getCell(5);
					HSSFCell trxTime = hssfRow.getCell(12);
					HSSFCell txtState = hssfRow.getCell(6);
					sacReceiveBankRecon.setBankSerialNo(getValue(bankSerialNo));
					sacReceiveBankRecon.setPayconType("2");
					sacReceiveBankRecon.setCurrencyType("CNY");
					sacReceiveBankRecon.setPayAmount(getValue(payAmount));
