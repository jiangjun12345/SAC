...
public class PageTagNum extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		while (en.hasMoreElements()) {
			...
			if (value.equals(""))
				continue;
			if(paramsStr.indexOf("&" + key + "=" + value)==-1){
	...
	public int doEndTag() throws JspException {
		...
		reStr.append("<font color='#3a74ae'>第<b>" + intPageNo + "/"
				+ totalPages + "</b>页");
		...
		if (totalPages < 2) {
		  ...
		  reStr.append("<script type='text/javascript'>");  
		  reStr.append("$(function(){$('#pageNo').change(function(){var pageNo=$('#pageNo').val(); window.location.href='");  
		  reStr.append(urlName+"?pageSize="+pageSize+paramsStr+"&pageNo='+pageNo;});});");  
		  reStr.append("</script>");  

		
		reStr.append("<select name='pageNo' id='pageNo' align='absmiddle' ");
		reStr.append("class='sel-splitpage' >");
		//reStr.append("onChange='submitForm();'>");
		if (totalPages == 0) {
