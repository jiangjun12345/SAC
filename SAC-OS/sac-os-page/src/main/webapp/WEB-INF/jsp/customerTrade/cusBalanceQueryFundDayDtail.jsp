...
<script type="text/javascript">
	$(function(){
		...
		$("#download").click(function(){
			$("#cusBalanceForm").attr("action","${ctx}/cusBalanceFundDayDownload").submit();
			$("#cusBalanceForm").attr("action","${ctx}/cusBalanceQueryFundDayDtail");
		});
		//点击退回按钮 
		$("#back").click(function(){
...
<title>客户每日余额明细查询</title>
</head>

<body>
	<div style="display: none;">
		<form id="cusBalanceQueryBackForm" action="cusBalanceFundDayQueryBack" method="get">
			<input type="text" id="cusNo" name="cusNo" value="${cusNo}"/>
			<input type="text" id="cusName" name="cusName"  value="${cusName}"/>
			<input type="text" id="bussType" name="bussType" value="${bussType}"/>
			<input type="text" id="childAccType" name="childAccType" value="${childAccType}"/>
			<input type="text" id="pageNo2" name="pageNoBack" value="${pageNoBack}"/>
	...
	<div class="content">
	 	<div class="con ">
	 		<form id="cusBalanceForm" action="cusBalanceQueryFundDayDtail" method="post">
	 		<input type="hidden" name="cusNo" value="${cusNo}"/>
			<input type="hidden" name="cusName"  value="${cusName}"/>
			<input type="hidden" name="bussType" value="${bussType}"/>
			<input type="hidden" name="childAccType" value="${childAccType}"/>
			<input type="hidden" name="pageNoBack" value="${pageNoBack}"/>
	 			...
	 			<div class="table fontcolor4 fontsize1 fontfamily2">
	 				<table width="100%" border="0" cellpadding="0" cellspacing="0">
	 					<tr>
	 						...
	 						<td>
	 							<input type="text" class="txt2" id="startDate" name="startDate" disabled="disabled" value="${beginDate}" 
	 								onfocus="WdatePicker({readOnly:true,maxDate:'#F{$dp.$D(\'endDate\')||\'%y-%M-%d\'}',dateFmt:'yyyy-MM-dd'})"/>
	 						</td>
	 						<td align="right">凭证时间(止):</td>
	 						<td>
								<input type="text" class="txt2" id="endDate" name="endDate" disabled="disabled"  value="${beginDate}" 
	 			...
	 			<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${totalCount}" pageNo="${pageNo}" url="/cusBalanceQueryFundDayDtail"/>
						</td>
					</tr>
				</table>
	 		</div>
