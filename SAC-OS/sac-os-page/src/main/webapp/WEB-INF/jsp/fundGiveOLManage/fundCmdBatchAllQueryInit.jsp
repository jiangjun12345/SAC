...
function queryMsg(){
	$("#subForm").attr("action","${ctx}/fundCmdBatchAllQueryInit").submit();
}
function downloadAll(){
	
	if (window.confirm("确定下载？")) {
		$("#subForm").attr("action","${ctx}/downloadAllToExcel").submit();
	}
}
...
function clearText(){
	...
	document.getElementById("batchState").value="";
}
</script>
</head>
<body>
<!-- Body -->
    ...
    <div class="content">
      ...
      <div class="con ">
				<form:form id="subForm" action="${ctx}/fundCmdBatchAllQueryInit" method="POST" >   	    
					<div class="table fontcolor4 fontsize1 fontfamily2">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr>
								...
								<td width="10%" align="right">出款状态：</td>
								<td width="25%"><select id="batchState" name="batchState"
									class="select1">
										<option value="">全部</option>
										<c:forEach items="${batchStateList}" var="sys">
												<option value="${sys.dicValue}" <c:if test="${sys.dicValue == batchState}"> selected="selected"</c:if>>${sys.dicDesc}</option>
										</c:forEach>
								</select>
								</td>
							</tr>
        ...
        <table id = "table1"  width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<table width="100%" cellspacing="0" cellpadding="0" height="24">
									<td align="right" height="24">划款金额总计:${totalAmount}
									</td>
									<td align="right" height="24"><easipay:pageNum pageSize="${pageSize}" count="${count}" pageNo="${pageNo}" url="/fundCmdBatchAllQueryInit"/>
									</td>
								</tr>
							</table>
							</div>
    		...
    		<div >
						<table  align="right">
								...
								<td align="left"><input name="submitbtn" type="button" 
									...
									id="submitbtn" onclick="downloadAll();"/>
								</td>
								</tr>
							</table>
							</div>
