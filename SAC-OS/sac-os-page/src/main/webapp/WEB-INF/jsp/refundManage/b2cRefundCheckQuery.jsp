...
<script type="text/javascript">
	$(function(){
		/* 下拉选项默认赋值  */
		var crtCurrency = "${sacB2cExrefundApply.crtCurrency}";/* 购汇币种 */
		$('select[name="crtCurrency"]').find("option[value="+crtCurrency+"]").attr("selected",true);
		
		var exBankCode = "${sacB2cExrefundApply.exBankCode}";/* 购汇银行 */
		$('select[name="exBankCode"]').find("option[value="+exBankCode+"]").attr("selected",true);
		...
		$("#download").click(function(){
			$("#refundTrxForm").attr("action","${ctx}/b2cRefundCheckDownloadToExcel").submit();
			$("#refundTrxForm").attr("action","${ctx}/b2cRefundCheckQuery");
		});
	});
	...
	function clearText(){
		...
		document.getElementById("crtCurrency").value = "";
		document.getElementById("exBankCode").value = "";
	}
	...
	function batchWipe() {
		...
		if (count > 0) {
			if (window.confirm("确定复核？")) {
				$("#refundTrxForm").attr("action","${ctx}/b2cRefundAudit?applyIds="+boxValues).submit();
			}
		}
...
<body>
	...
	<div class="content">
	 	<div class="con ">
	 		<form id="refundTrxForm" action="b2cRefundCheckQuery" method="post">
	 			<div class="table fontcolor4 fontsize1 fontfamily2">
	 				<table width="100%" border="0" cellpadding="0" cellspacing="0">
	 					<tr>
	 						<td align="right">经办日期(起):</td>
	 						<td>
	 							<input type="text" class="txt2" id="startDate1" name="startDate1" value="${startDate1}" 
	 								onfocus="WdatePicker({readOnly:true,maxDate:'#F{$dp.$D(\'endDate1\')||\'%y-%M-%d\'}',dateFmt:'yyyyMMdd'})"/>
	 						</td>
	 						<td align="right">经办日期(止):</td>
	 						<td>
	 							<input type="text" class="txt2" id="endDate1" name="endDate1" value="${endDate1}" 
	 								onfocus="WdatePicker({readOnly:true,minDate:'#F{$dp.$D(\'startDate1\')}',dateFmt:'yyyyMMdd',maxDate:'%y-%M-%d'})"/>
	 						</td>
	 					...
	 					<tr>
	 						...
	 						<td>
	 							<select id="crtCurrency" name="crtCurrency" class="select1" >
	 						...
	 						<td align="right">购汇银行：</td>
	 						<td>
								<select name="exBankCode" class="select1" id="exBankCode">
									<option value="" >全部</option>
	 						...
	 						<td colspan="1"><span id="validateDateMsg" style="color: red;"></span></td>
							<td align="left"><input id="query" class="bluebtn" type="button" value="查询" >
							<input name="clearBtn" type="button" value="清除"
								class="bluebtn" id="clearBtn" onclick="clean();clearText();" />
							</td>
							<td align="left"><input id="download" class="bluebtn" type="button" value="下载">
							<input name="batchWipeClick" type="button" value="勾选复核"
	 			...
	 			<table width="1500px" border="0" cellpadding="0" cellspacing="0">
	                ...
	                <c:forEach var="cmdDetail" items="${commandList}" varStatus="status">
	                	<c:choose>
							...
							<td class="fontfamily1">${cmdDetail.exBankName}</td>
							<td class="fontfamily1">${cmdDetail.crtCurrencyName}</td>
							<td class="fontfamily1">${cmdDetail.purchStateName}</td>
							<td class="fontfamily1">${cmdDetail.remStateName}</td>
							<td class="fontfamily1">
								<p><fmt:formatDate value="${cmdDetail.operateTime}" pattern="yyyy/MM/dd HH:mm:ss"/></p>
							</td>
	 			...
	 			<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${totalCount}" pageNo="${pageNo}" url="/b2cRefundCheckQuery"/>
						</td>
					</tr>
				</table>
	 		</div>
