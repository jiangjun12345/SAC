...
<script type="text/javascript">
	$(function(){
		/* 下拉选项默认赋值  */
		var applyState = "${sacB2cExrefundApply.applyState}";/* 退款状态 */
		$('select[name="applyState"]').find("option[value="+applyState+"]").attr("selected",true);
		...
		$("#download").click(function(){
			$("#refundTrxForm").attr("action","${ctx}/b2cRefundDownloadToExcel").submit();
			$("#refundTrxForm").attr("action","${ctx}/b2cRefundQuery");
		});
	});
	...
	function clearText(){
		...
		document.getElementById("applyState").value = "";
	}
	...
	function batchWipe() {
		...
		if (count > 0 && !failFlag) {
			if (window.confirm("确定下载？")) {
				$("#refundTrxForm").attr("action","${ctx}/b2cRefundDownloadToExcel?wpIds="+boxValues).submit();
			}
		}
...
<body>
	...
	<div class="content">
	 	<div class="con ">
	 		<form id="refundTrxForm" action="b2cRefundQuery" method="post">
	 			<div class="table fontcolor4 fontsize1 fontfamily2">
	 				<table width="100%" border="0" cellpadding="0" cellspacing="0">
	 					<tr>
	 						...
	 						<td>
	 							<select id="applyState" name="applyState" class="select1" >
	 					...
	 					<tr>
							...
							<td><input name="clearBtn" type="button" value="清除"
								...
								<input id="download" class="bluebtn" type="button" value="下载"></td>
	 					</tr>
	 					<tr>
	 			...
	 			<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${totalCount}" pageNo="${pageNo}" url="/b2cRefundQuery"/>
						</td>
					</tr>
				</table>
	 		</div>
