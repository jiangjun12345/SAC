...
function queryDetailForReversal(id,reversalStatus,trxState){
	...
	if(reversalStatus=='S'||trxState=='N'){
...
function reversalConfirm(){
	if(window.confirm("确定冲正审核通过?")){
		$("#confirmClick").attr("disabled",true);
		var trxSerialNo = document.getElementById('trxSerialNoDetail').value;
	    var url = "reversalConfirm";
		$.ajax( {
			url : url,
			cache : false,
			async : false,
...
function queryDetailForReversalFail(trxSerialNo){
	if(window.confirm("确定冲正审核不通过?")){
	    var url = "reversalConfirm";
		$.ajax( {
			url : url,
			cache : false,
...
<!-- Body -->
    ...
    <div class="content">
      ...
      <div class="con ">
				<form:form id="subForm" commandName="sacOtrxInfo" action="${ctx}/reversalDealCheck" method="POST" >   	    
								...
								<td width="30%">
									<select id="trxState" name="trxState" class="select1" >
										...
										<c:forEach items="${trxStateList}" var="sys">
											<option value="${sys.dicValue}" <c:if test="${sys.dicValue}"> selected="selected"</c:if>><c:if test="${sys.dicValue == 'N'}"> 待支付</c:if><c:if test="${sys.dicValue == 'S'}"> 支付成功</c:if></option>
										</c:forEach>
									</select>
								</td>	 --%>	
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                ...
                                <c:forEach items="${sacOtrxInfoList}" var="item" varStatus="st">
                                    ...
                                    <td class="fontfamily1"><c:if test="${item.bussType eq '50'}">外汇通</c:if><c:if test="${item.bussType eq '20'}">航付通</c:if><c:if test="${item.bussType eq '30'}">跨境B2C</c:if><c:if test="${item.bussType eq '40'}">代收付</c:if><c:if test="${item.bussType eq '60'}">清算</c:if></td>
                                    <td class="fontfamily1">${item.trxType}</td>
                                    <td class="fontfamily1"><c:if test="${item.trxState eq 'N'}">待支付</c:if><c:if test="${item.trxState eq 'S'}">支付成功</c:if></td>
                                    <td class="fontfamily1"><c:if test="${item.reversalStatus eq 'N'}">未冲正</c:if><c:if test="${item.reversalStatus eq 'D'}">待审核</c:if><c:if test="${item.reversalStatus eq 'S'}">已冲正</c:if></td>
                                    <td class="fontfamily1"><fmt:formatDate value="${item.createTime}" pattern="yyyy/MM/dd HH:mm:ss"/></td>
                                    <td class="fontfamily1">
	                                   <c:if test="${item.reversalStatus eq 'D' && item.trxState eq 'S'}">
	                                   <input type="button" id="reversalButton" value="审核通过"
										onClick="showidTranaction('reversalDetailDiv');queryDetailForReversal('${item.id}','${item.reversalStatus}','${item.trxState}');">&nbsp;
										<input type="button" id="reversalButton" value="审核不通过"
										onClick="queryDetailForReversalFail('${item.trxSerialNo}');">&nbsp;
