...
function downSubmit(){
	$("#feeOutCheckInitid").attr("target","newWin");
	$("#feeOutCheckInitid").attr("action","${ctx}/feeOutCommonDownload").submit();
	$("#feeOutCheckInitid").attr("action","${ctx}/feeOutCheckInit");
	$("#feeOutCheckInitid").attr("target","");
}

function showDetail(){
var url="feeOutCommonDownload"; //转向网页的地址;
...
//线上审核操作
function checkSucc(checkStatus,id,craccBankNodeCode,craccBankName,payAmount,craccCardId,craccCusName,index){//审核结果
	if(confirm("确认？")){
...
<body>
	<div class="content">
		<div class="con ">
			<form id="feeOutCheckInitid" name="feeOutCheckInitid" action="feeOutCheckInit" method="POST">
				<div class="table fontcolor4 fontsize1 fontfamily2">
	            <table  width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td width="12%" align="right">结转日期(起)：</td>
						<td>
						...
						<td width="12%" align="right">结转日期(止)：</td>
						<td>
			...
			<div width="500px" style="overflow: scroll;">
				<table width="1400px" border="0" cellpadding="0" cellspacing="0">
					<tr height="35" bgcolor="#cccccc">
						...
						<th class="border">结转金额</th>
						<th class="border">收款银行</th>
						<th class="border">收款银行名称</th>
						<th class="border">收款方组织机构号</th>
					...
					<c:forEach items="${sacOtrxInfoList}" var="item" varStatus="st">
						...
						<td class="fontfamily1" align="left">
							<c:if test="${item.memo eq '2'}">
							    <input id="checkSucc${st.index}" class="bluebtn" style="width: 90px;" type="button" value="审核通过" onclick="checkSucc('1','${item.id}','${item.craccNodeCode}','${item.craccBankName}','${item.payAmount}','${item.craccCardId}','${item.craccCusName}',${st.index});" />
								<input id="checkFail${st.index}" class="bluebtn"  type="button" value="审核不通过" onclick="checkSucc('3','${item.id}','${item.craccNodeCode}','${item.craccBankName}','${item.payAmount}','${item.craccCardId}','${item.craccCusName}',${st.index});" />
							</c:if>
						</td>
				...
				<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${count}" pageNo="${pageNo}" url="/feeOutCheckInit" />
						</td>
					</tr>
				</table>
			</div>
