...
function downSubmit(){
	$("#feeInCheckInitid").attr("target","newWin");
	$("#feeInCheckInitid").attr("action","${ctx}/feeInCommonDownload").submit();
	$("#feeInCheckInitid").attr("action","${ctx}/feeInCheckInit");
	$("#feeInCheckInitid").attr("target","");
}

function showDetail(){
var url="feeInCommonDownload"; //转向网页的地址;
...
//线上审核操作
function checkSucc(checkStatus,id,draccBankNodeCode,draccBankName,payAmount,draccCardId,draccCusName,index){//审核结果
	if(confirm("确认？")){
...
<body>
	<div class="content">
		<div class="con ">
			<form id="feeInCheckInitid" name="feeInCheckInitid" action="feeInCheckInit" method="POST">
			...
			<div width="500px" style="overflow: scroll;">
				<table width="1400px" border="0" cellpadding="0" cellspacing="0">
					<tr height="35" bgcolor="#cccccc">
						...
						<th class="border">付款方组织机构号</th>
						<th class="border">付款客户名称</th>
					...
					<c:forEach items="${sacOtrxInfoList}" var="item" varStatus="st">
						...
						<td class="fontfamily1" align="left">
							<c:if test="${item.memo eq '2'}">
							    <input id="checkSucc${st.index}" class="bluebtn" style="width: 90px;" type="button" value="审核通过" onclick="checkSucc('1','${item.id}','${item.draccNodeCode}','${item.draccBankName}','${item.payAmount}','${item.draccCardId}','${item.draccCusName}',${st.index});" />
								<input id="checkFail${st.index}" class="bluebtn"  type="button" value="审核不通过" onclick="checkSucc('3','${item.id}','${item.draccNodeCode}','${item.draccBankName}','${item.payAmount}','${item.draccCardId}','${item.draccCusName}',${st.index});" />
							</c:if>
						</td>
				...
				<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${count}" pageNo="${pageNo}" url="/feeInCheckInit" />
						</td>
					</tr>
				</table>
			</div>
