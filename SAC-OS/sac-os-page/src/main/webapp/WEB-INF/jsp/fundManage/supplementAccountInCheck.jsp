...
//线上审核操作
function checkSucc(checkStatus,id,craccNodeCode,craccNo,craccCardId,bussType,payAmount,index,etrxSerialNo,draccNo){//审核结果
	//付款方银行为线上4家银行
	if(confirm("确认审核通过？")){
...
<body>
	<div class="content">
		<div class="con ">
			<form id="AccountInCheckid" name="AccountInCheckid" action="accountInCheck" method="POST">
					...
					<c:forEach items="${sacOtrxInfoList}" var="item" varStatus="st">
						...
						<td class="fontfamily1" align="left">
							<c:if test="${item.memo eq '2'}">
							    <input id="checkSucc${st.index}" class="bluebtn" style="width: 90px;" type="button" value="审核通过" onclick="checkSucc('1','${item.id}','${item.craccNodeCode}','${item.craccNo}','${item.craccCardId}','${item.bussType}','${item.payAmount}','${st.index}','${item.etrxSerialNo}','${item.draccNo}');" />
				...
				<table width="100%" cellspacing="0" cellpadding="0" height="24">
					<tr>
						<td align="right" height="24">
							<easipay:pageNum pageSize="${pageSize}" count="${count}" pageNo="${pageNo}" url="/accountInCheck" />
						</td>
					</tr>
				</table>
			</div>
