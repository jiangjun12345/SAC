...
$.formValidator = 
{
	...
	inputValid : function(returnObj)
	{
		...
		var blank = setting.blank,blankError = false;
		switch(sType)
		{
			case "text":
			case "hidden":
			...
			case "textarea":
				if (setting.type == "size"){
					blank = setting.blank;
					if(!blank.allBlank){
						blankError = (val.length==0);
					}
					if(blankError && blank.blankError){
						returnObj.errormsg= blank.blankError;
					}
				}
			case "file":
				if (setting.type == "size") {
					...
					if(blankError && blank.blankError){returnObj.errormsg= blank.blankError}
				}
			case "checkbox":
			case "select-one":
			case "select-multiple":
	...
	return this.each(function(e)
	{
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			{	
				...
				if (stype == "password" || stype == "text" || stype == "file") {this.validValueOld = jqobj.val()};
				if(stype == "textarea"){
					if(jqobj.val()==setting.defaultTxt) {
						jqobj.val("");
					}
				};
			});
...
var inputValidator_setting = 
{
	...
	blank:{allBlank:true,allBlankError:null}
};
